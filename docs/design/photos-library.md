...
1. **디스크 썸네일 캐시 공유?** → **공유 안 함.** 에셋 썸네일은 PHImageManager 자체 캐시에 맡김(메모리 NSCache만 공유). 기존 3.3GB 샤딩 디스크 캐시엔 에셋 미저장.
2. **에셋 내보내기 범위.** → **(a)만.** Phase 2에서 PhotoKit 원본 데이터 추출 → 사용자가 고른 폴더로 실제 파일 저장. ("사진앱에서 열기"는 범위 밖.)
3. **비디오.** → **이미지만.** 페치 predicate `mediaType == .image`. 비디오는 별도 과제.
4. **즐겨찾기 의미.** → **분리.** Lumen 메타는 localIdentifier(합성 URL) 키로 독립. 사진앱 `isFavorite`와 양방향 동기화는 Phase 4(선택).
5. **샌드박스/배포.** → **추후 결정.** 1차는 비샌드박스 ad-hoc(usage string만으로 권한 다이얼로그 동작). 노타라이즈 시 엔타이틀먼트 정리.
